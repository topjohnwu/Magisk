...
object Codegen {
    ...
    private val RANDOM = if (RAND_SEED != 0) Random(RAND_SEED.toLong()) else SecureRandom()
    ...
    fun genStubManifest(srcDir: File, outDir: File): String {
        ...
        val names = mutableListOf<String>().run {
            addAll(c1)
            addAll(c2.subList(0, 10))
            addAll(c3.subList(0, 10))
            val set = mutableSetOf<String>()
            forEach { set.add(it.toLowerCase(Locale.ROOT)) }
            set
        }.toMutableList()
        names.shuffle(RANDOM)

        var idx = 0
        fun genCmpName() = "${names[idx++]}.${names[idx++]}"
