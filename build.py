...
def build_binary(arguments):
    header('* Building Magisk binaries')

    # Force update logging.h timestamp to trigger recompilation
    os.utime(os.path.join('core', 'jni', 'include', 'logging.h'), None)

    debug_flag = '' if arguments.release else '-DMAGISK_DEBUG'
    cflag = 'MAGISK_FLAGS=\"-DMAGISK_VERSION=\\\"{}\\\" -DMAGISK_VER_CODE={} {}\"'.format(arguments.versionString,
                                                                                          arguments.versionCode,
                                                                                          debug_flag)

    # Prebuild
    proc = subprocess.run('{} -C core PRECOMPILE=true {} -j{}'.format(ndk_build, cflag, multiprocessing.cpu_count()),
                          shell=True)
    ...
    for arch in ['arm64-v8a', 'armeabi-v7a', 'x86', 'x86_64']:
        ...
        with open(os.path.join('out', arch, 'dump.h'), 'w') as dump:
            ...
            with open(os.path.join('out', arch, 'magisk'), 'rb') as binary:
                dump.write('const uint8_t magisk_dump[] = "')
                dump.write(''.join("\\x{:02X}".format(c) for c in lzma.compress(binary.read(), preset=9)))
                dump.write('";\n')
...
def zip_main(arguments):
    ...
    with zipfile.ZipFile(unsigned, 'w', compression=zipfile.ZIP_DEFLATED, allowZip64=False) as zipf:
        ...
        # APK
        source = os.path.join('out', 'app-release.apk' if arguments.release else 'app-debug.apk')
        ...
        with open(source, 'r') as script:
            ...
            util_func = script.read().replace(
                '#MAGISK_VERSION_STUB',
                'MAGISK_VER="{}"\nMAGISK_VER_CODE={}'.format(arguments.versionString, arguments.versionCode))
    ...
    output = os.path.join('out', 'Magisk-v{}.zip'.format(arguments.versionString))
...
def sign_adjust_zip(unsigned, output):
    ...
    if os.name != 'nt' and not os.path.exists(os.path.join('ziptools', 'zipadjust')):
        ...
        # Compile zipadjust
        proc = subprocess.run(
            'gcc -o {0} {0}_{1} -lz'.format(os.path.join('ziptools', 'zipadjust'), os.path.join('src', '*.c')),
            shell=True)
        if proc.returncode != 0:
    ...
    public_key = os.path.join('ziptools', 'public.certificate.x509.pem')
    private_key = os.path.join('ziptools', 'private.key.pk8')

    signed = tempfile.mkstemp()[1]

    # Unsigned->signed
    proc = subprocess.run(['java', '-jar', jarsigner,
                           public_key, private_key, unsigned, signed])
    if proc.returncode != 0:
    ...
    # Adjusted -> output
    proc = subprocess.run(['java', '-jar', jarsigner,
                           "-m", public_key, private_key, adjusted, output])
    if proc.returncode != 0:
...
def cleanup(arguments):
    if len(arguments.target) == 0:
        arguments.target = ['binary', 'java', 'zip']

    if 'binary' in arguments.target:
...
if len(sys.argv) == 1 or re.match('--?h(?:elp)?', sys.argv[1]):
    parser.print_help()
    sys.exit(1)
