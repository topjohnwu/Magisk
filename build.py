...
def sign_zip(unsigned, output, release):
    ...
    if release:
        proc = execv(
            ['java', '-jar', zipsigner, keystore, config['keyStorePass'], config['keyAlias'],
             config['keyPass'], unsigned, output])
    else:
...
def build_binary(args):
    ...
    # Basic flags
    base_flags = 'MAGISK_VERSION=\"{}\" MAGISK_VER_CODE={}'.format(config['version'],
                                                                   config['versionCode'])
    if not args.release:
...
def zip_main(args):
    ...
    with zipfile.ZipFile(unsigned, 'w', compression=zipfile.ZIP_DEFLATED, allowZip64=False) as zipf:
        ...
        # APK
        source = os.path.join(config['outdir'],
                              'app-full-release.apk' if args.release else 'app-full-debug.apk')
        ...
        with open(source, 'r') as script:
            ...
            util_func = script.read().replace('#MAGISK_VERSION_STUB',
                                              'MAGISK_VER="{}"\nMAGISK_VER_CODE={}'.format(
                                                  config['version'], config['versionCode']))
    ...
    output = os.path.join(config['outdir'],
                          'Magisk-v{}.zip'.format(config['version']) if config['prettyName'] else
...
def zip_uninstaller(args):
    ...
    output = os.path.join(config['outdir'], 'Magisk-uninstaller-{}.zip'.format(
...
binary_parser.add_argument('target', nargs='*',
                           help='Support: magisk, magiskinit, magiskboot, busybox, b64xz. Leave empty to build all.')
...
snet_parser = subparsers.add_parser('snet', help='build snet extension for Magisk Manager')
...
clean_parser.add_argument('target', nargs='*',
                          help='Support: native, java. Leave empty to clean all.')
