...
template <typename T>
auto_cast_wrapper<T> auto_cast(T *p) {
...
bool str_eq(string_view a, rust::Str b) {
    return a.size() == b.size() && memcmp(a.data(), b.data(), a.size()) == 0;
}

auto hashtab_find(hashtab_t h, Str key) {
    array<char, 256> buf{};
    copy_str(buf, key);
...
void list_for_each(Node *node_ptr, const Func &fn) {
    auto cur = node_ptr;
    while (cur) {
        auto next = cur->next;
        fn(cur);
...
template <class Node, class Func>
Node *list_find(Node *node_ptr, const Func &fn) {
...
template <class Node, class Func>
void hash_for_each(Node **node_ptr, int n_slot, const Func &fn) {
...
template <class Func>
void hashtab_for_each(hashtab_t htab, const Func &fn) {
    hash_for_each(htab->htable, htab->size, fn);
}

template <class Func>
void avtab_for_each(avtab_t *avtab, const Func &fn) {
    hash_for_each(avtab->htable, avtab->nslot, fn);
}

template <class Func>
void for_each_attr(hashtab_t htab, const Func &fn) {
...
bool is_redundant(avtab_ptr_t node) {
