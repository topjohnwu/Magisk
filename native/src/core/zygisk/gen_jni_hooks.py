...
class JNIMethod:
    ...
    def arg_list_name(self) -> str:
        return "env, clazz, " + ", ".join(x.name for x in self.args)

    def arg_list_cpp(self) -> str:
        return "JNIEnv *env, jclass clazz, " + ", ".join(
            x.cpp() for x in self.args
        )
    ...
    def shorty(self):
        args = "".join(x.type.shorty() for x in self.args)
...
def gen_jni_def(field: str, methods: list[JNIHook]):
    decl = "static std::remove_reference_t<decltype(Self{}())> table {{"
    for i, m in enumerate(methods):
        ...
        orig_fn_ptr = f"Self{{}}()[{i}].fnPtr"
        decl += m.body(orig_fn_ptr)
        decl += ind(2) + "}"
        decl += ind(1) + "},"
    decl += ind(0) + "}};"
    return (
        f"\nauto &{field} = []<typename Self>(this Self) -> std::array<JNINativeMethod, {len(methods)}> & {{\n"
        + indent(decl, "    ")
        + "\n    return table;\n}();\n"
    )


with Path(__file__).with_name("jni_hooks.cpp").open("w") as f:
    f.write("// Generated by gen_jni_hooks.py\n")
    f.write("module;\n#include <jni.h>\n\n")
    f.write("module core:zygisk.jni;\nimport std;\nimport :zygisk;\n")
    f.write(
        gen_jni_def(
            "fork_app_methods",
            [
