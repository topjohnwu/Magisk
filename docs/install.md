...
- If you plan not to use custom recoveries, you will have to extract `RECOVERY_RAMDISK.img` from your firmware. Follow the instructions for boot image patching above, but use the `RECOVERY_RAMDISK.img` file instead of a boot image.
- To flash the patched image to your device, here is the fastboot command: <br>
