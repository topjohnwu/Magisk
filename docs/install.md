...
To verify whether or not Magisk can be installed in your Samsung device, you first must check the OEM Lock and KnoxGuard (RMM) status. To do so, go to Settings-Device Care-Maintenance Mode and open it, then boot your device in Download mode with its key combo.
