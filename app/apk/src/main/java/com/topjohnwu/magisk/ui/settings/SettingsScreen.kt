...
private fun CustomizationSection(viewModel: SettingsViewModel) {
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        ...
        val themeEntries = remember(themes) { themes.map { it.themeName } }
        val selectedTheme = Theme.selected
        val safeThemeIndex = themes.indexOf(selectedTheme).coerceAtLeast(0)
        var themeIndex by remember { mutableIntStateOf(safeThemeIndex) }
        SuperDropdown(
            ...
            onSelectedIndexChange = { index ->
                themeIndex = index
                Config.themeOrdinal = themes[index].ordinal
