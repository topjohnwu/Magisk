...
# It will then call system_patch.sh to patch the System if system mode detected
# Otherwise it will then call boot_patch.sh to patch the boot image
#
##########################################################################################
...
check_data
get_flags
$SYSTEMMODE || find_boot_image

$SYSTEMMODE && is_mounted /system || mount /system 

[ -z $SYSTEMMODE ] && [ -z $BOOTIMAGE ] && abort "! Unable to detect target image"
[ -z $SYSTEMMODE ] && ui_print "- Target image: $BOOTIMAGE"
...
system_patching() {
  ...
  $IS64BIT && mv -f $MAGISKBIN/magiskinit64 $MAGISKBIN/magiskinit || rm -f $MAGISKBIN/magiskinit64

  # Source the system patcher
  . $MAGISKBIN/system_patch.sh

  # install magisk manager apk
  install_manager_apk
}

$SYSTEMMODE && system_patching || boot_patching

cd /
# Cleanups
$BOOTMODE && $SYSTEMMODE || recovery_cleanup
rm -rf $TMPDIR
